...
def plotNumberedGridKMNew(latitude, longitude, radius, n, zoomlevel, zlplotlabel, kring):
    ...
    with open('./public/data/ImpactZones.csv', mode='w', newline='') as impactzone_file:
        ...
        while i < len(DC_array):
            ...
            thisimpactzonebydc=[]
            thisimpactzonebydc.append(DC_array[i][0])
            thisimpactzonebydc.append(piz[0])
            thisimpactzonebydc.append(piz[1])
            thisimpactzonebydc.append(piz[2])
            thisimpactzonebydc.append(piz[3])
            thisimpactzonebydc.append(piz[4])
            thisimpactzonebydc.append(piz[5])
            thisimpactzonebydc.append(siz[0])
            thisimpactzonebydc.append(siz[1])
            thisimpactzonebydc.append(siz[2])
            thisimpactzonebydc.append(siz[3])
            thisimpactzonebydc.append(siz[4])
            thisimpactzonebydc.append(siz[5])
            ImpactZone_array.append(thisimpactzonebydc)
            i += 1
    ...
    with open('./public/data/L1Plot.csv', mode='w', newline='') as l1plot_file:
        ...
        l1plot_csv_writer.writerow(['L1Plot', 'dc1', 'dc2', 'dc3', 'zl plot1', 'zl plot2', 'zl plot3', 'zl plot4'])
        i=1
        l1num=0
        while i < 6:
            L1Plot_Number = 'L1' + str(l1num).zfill(4)
            thisl1plotarray= []
            point1_name = DC_array[i][0]
            point1 = DC_array[i][1]
            if i == 5:
                point2_name = DC_array[1][0]
                point2_name = DC_array[1][0]
            else:    
                point2_name = DC_array[i+1][0]
                point2 = DC_array[i+1][1]
            point3_name = DC_array[0][0]
            point3 = DC_array[0][1]
            ...
            zlplotsinl1plots=[]
            #print(L1Plot_Number)
            zlplotsinl1plots = find_zlplots_in_l1_plots(point1_name, point2_name, point3_name, ImpactZone_array, ZLPlot_array)
            #print(thisl1plotarray)
            l1plot_csv_writer.writerow([L1Plot_Number,  point1_name, point2_name, point3_name, zlplotsinl1plots[0], zlplotsinl1plots[1], zlplotsinl1plots[2], zlplotsinl1plots[3]])
            i += 1
            l1num += 1
            ...
            while j < len(hexagon[i]):
                ...
                while k < len(hexagon[i][j]):
                    ...
                    if i == 2:
                        ...
                        L1Plot_array.append(thisl1plotarray) 
                        l1num += 1
                        #print(i, j, k, point1_name, point2_name, point3_name)
                        zlplotsinl1plots=[]
                        #print(L1Plot_Number)
                        zlplotsinl1plots = find_zlplots_in_l1_plots(point1_name, point2_name, point3_name, ImpactZone_array, ZLPlot_array)
                        l1plot_csv_writer.writerow([L1Plot_Number, point1_name, point2_name, point3_name, zlplotsinl1plots[0], zlplotsinl1plots[1], zlplotsinl1plots[2], zlplotsinl1plots[3]])
...
def find_zlplots_in_l1_plots(DC1, DC2, DC3, ImpactZone_array, ZLPlot_array):
    i=0
    siz=[]
    sizsorted=[]
    #print('impactzone array')
    #print(DC1, DC2, DC3)
    while i < len(ImpactZone_array):
        if ((ImpactZone_array[i][0] == DC1) | (ImpactZone_array[i][0] == DC2) | (ImpactZone_array[i][0] == DC3)):
            siz.append(ImpactZone_array[i][7])
            siz.append(ImpactZone_array[i][8])
            siz.append(ImpactZone_array[i][9])
            siz.append(ImpactZone_array[i][10])
            siz.append(ImpactZone_array[i][11])
            siz.append(ImpactZone_array[i][12])
        i += 1
    #print('siz')
    #print(siz)
    sizsorted=sorted(siz, key=siz.count, reverse=True)
    #print(DC1, DC2, DC3, sizsorted[0])  
    surrpiz=[]
    surrpiz=get_surr_primary_zones(sizsorted[0], ZLPlot_array) 
    surrpiz.append(siz)
    return surrpiz
                       

def get_surr_primary_zones(siz, ZLPlot_array):
    i=0
    point1 = ''
    point2 = ''
    point3 = ''
    surrpiz=[]
    while i < len(ZLPlot_array):
        if ZLPlot_array[i][0] == siz:
            point1 = ZLPlot_array[i][1]
            point2 = ZLPlot_array[i][3]
            point3 = ZLPlot_array[i][5]
        i += 1
    i=0
    while i < len(ZLPlot_array):
        if (((point1 == ZLPlot_array[i][1]) | (point1 == ZLPlot_array[i][3]) | (point1 == ZLPlot_array[i][5])) & ((point2 == ZLPlot_array[i][1]) | (point2 == ZLPlot_array[i][3]) | (point2 == ZLPlot_array[i][5]))):
            surrpiz.append(ZLPlot_array[i][0])
        if (((point1 == ZLPlot_array[i][1]) | (point1 == ZLPlot_array[i][3]) | (point1 == ZLPlot_array[i][5])) & ((point3 == ZLPlot_array[i][1]) | (point3 == ZLPlot_array[i][3]) | (point3 == ZLPlot_array[i][5]))):
            surrpiz.append(ZLPlot_array[i][0])
        if (((point2 == ZLPlot_array[i][1]) | (point2 == ZLPlot_array[i][3]) | (point2 == ZLPlot_array[i][5])) & ((point3 == ZLPlot_array[i][1]) | (point3 == ZLPlot_array[i][3]) | (point3 == ZLPlot_array[i][5]))):
            surrpiz.append(ZLPlot_array[i][0])
        i += 1    
    
    #print(list(set(surrpiz)))
    #remove duplicates and return
    return (list(set(surrpiz)))